...
exports.findAll = (req, res) => {
    DetalleVenta.findAll()
        ...
        .catch(err => {
            res.status(500).send({
                message: err.message || "Ocurrió un error al recuperar los Detalles de Ventas."
            });
        });
};
...
exports.deleteAll = (req, res) => {
    DetalleVenta.destroy({ where: {}, truncate: false })
        .then(nums => { res.send({ message: `${nums} Detalles eliminados correctamente.` }); })
        .catch(err => {
            res.status(500).send({
                message: err.message || "Ocurrió un error eliminando los Detalles."
            });
        });
};
